...
/** Public report lookup — a parent provides a date and the player's
 *  MyKid number. Matches against EITHER the original session date or the
 *  date the evaluation was actually completed/saved — these can differ
 *  (a coach might fill in Monday's evaluation on Wednesday), and a parent
 *  has no way to know which one the system considers "the" date, so
 *  either should work. */
export async function getPublicPlayerPerformance(date, myKid) {
  ...
  const row = await queryOne(
    "SELECT * FROM evaluations WHERE (date = ? OR completed_date = ?) AND player_id = ? AND status = 'Completed'",
    [date, date, player.player_id]
  );
  if (!row) return null;
