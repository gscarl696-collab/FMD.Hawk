...
export async function getAttendanceForDate(token, date) {
  ...
  return rows
    .map((r) => ({ playerId: r.player_id, playerName: r.player_name, category: r.category, timeScanned: r.time_scanned, scannedBy: r.scanned_by }))
    .sort((a, b) => String(a.timeScanned).localeCompare(String(b.timeScanned)));
}
