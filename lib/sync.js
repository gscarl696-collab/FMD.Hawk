 ...
 *
 * Called from two places, both wrapping this same function:
 *   - api/cron/sync-form-submissions.js — the automatic daily run
 *   - the "Check for new registrations" action in the Admin Dashboard —
 *     an on-demand manual trigger for whenever waiting for the next
 *     scheduled run isn't good enough
 *
 * ---------------------------------------------------------------------
...
import { ageToCategory, playerQrCodeUrl } from './players.js';
import { addNotification } from './notifications.js';
import { getGoogleAuth, copyDriveFileToBlob, driveViewLink } from './drive.js';
 ...
 *  — see guardianPhone() below for how the correct one gets picked when
 *  both exist. */
const FIELD_HEADERS = {
  'Timestamp': 'timestamp',
...
/** Does the actual sync work and returns a small summary object. Throws
 *  on failure — callers decide how to report that (HTTP 500 for the cron
 *  route, a thrown error the dashboard shows as a toast for the manual
 *  button). */
export async function runFormSync() {
  ...
  if (!headerRow) return { synced: 0, checked: 0, message: 'Sheet is empty.' };
  ...
  return { synced: syncedCount, checked: rows.length };
}

/** Admin-only, on-demand version of the same sync — for whenever waiting
 *  for the next scheduled daily run isn't good enough (a parent says
 *  "I just submitted the form," and the admin wants it to show up now,
 *  not tomorrow at 4am). Same underlying logic as the cron job, just
 *  triggered by a button instead of a timer, with a real login check
 *  since this isn't locked behind Vercel's cron-only header the way the
 *  scheduled route is. */
export async function syncFormSubmissionsNow(token) {
