...
export async function updatePlayer(token, playerId, updates) {
  await requireAdmin(token);
  const existing = await queryOne('SELECT status FROM players WHERE player_id = ?', [playerId]);
  if (!existing) throw new Error('Player not found: ' + playerId);

  const sets = [];
  const args = [];

  // Digital ID's "Date Issued" reflects the day status last changed TO
  // Active — only stamp it on an actual transition, not every save.
  ...
  if (updates.age !== undefined) {
    sets.push('category = ?');
    args.push(ageToCategory(clampAge(updates.age)));
  }
