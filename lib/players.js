...
export async function updatePlayer(token, playerId, updates) {
  ...
  if (categoryChanged) {
    // A reissued ID is referenced by Attendance and Evaluation history, and
    // by the form-sync tracking table — all of it has to move to the new ID
    // together, in one transaction, or those records would silently become
    // orphaned (pointing at an ID that no longer exists on any player).
    await transaction([
      ...
      { sql: 'UPDATE synced_form_rows SET player_id = ? WHERE player_id = ?', args: [newPlayerId, playerId] }
    ]);
